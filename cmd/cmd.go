...
import (
	...
	"github.com/RachaelLuo/kex/pkg/server"
	"github.com/RachaelLuo/kex/pkg/utils"
	"github.com/RachaelLuo/kex/pkg/zone"
...
func NewCommand(name string, in io.Reader, out, errout io.Writer) *cobra.Command {
	...
	flags.IntVar(&o.config.Port, "port", 8080, "kex listen port")
	flags.StringVar(&o.config.KubeConfig, "kubeconfig", "", "Path to the kubeconfig file to use for CLI requests.")
	flags.StringVar(&o.config.NameSpace, "namespace", "kex-system", "If present, the namespace scope for this CLI request")
	flags.StringVar(&o.config.ClusterInfos, "cluster-info", "kex-cluster-info", "configmaps name used by kex")
	flags.StringVar(&o.config.LocalClusterInfos, "local-cluster-info", "", "Local cluster-info")
	flags.StringVar(&o.config.BasicAuthUser, "basic-auth-user", "admin", "kex basic auth user")
	flags.StringVar(&o.config.BasicAuthPassword, "basic-auth-password", "admin", "kex basic auth password")
	return cmd
}
